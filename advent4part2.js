...
function sortWord (str) {
  if (typeof str !== 'string') {
    throw 'type error: not string';
  }
  return _.reduce(str.split(''), function(sortedArr, value, index, arr){
    sortedArr.splice(_.sortedIndex(sortedArr, value), 0, value);
    return sortedArr;
  }, []).join('');
}
...
var numberOfValidPassphrases = _.reduce(processed, function(sum, currentPasswordsArr) {
  ...
  return sum + _.every(currentPasswordsArr, function(password) {
    var sortedPassword = sortWord(password);
    // console.log("sortedPassword...: ",sortedPassword);
    if (currentPasswordsTrie.contains(sortedPassword)) {
      return false;
    }
    currentPasswordsTrie.add(sortedPassword);
    return true;
  });
}, 0);
